...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  node_t *blk = NULL;
  for (node_t *cur = freelist->head; cur != NULL; cur = cur->next) {
    if (cur->end - cur->start >= blocksize) {
      blk = cur;
    }
  }
  if (blk == NULL) {
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  node_t * node = find_node(alloclist, pid);

  if (node != NULL) {
    remove_node(alloclist, node);

    node->blk.pid = 0;

    switch (policy) {
      case 1: // FIFO
        add_node_front(freelist, node);
      case 2: // BESTFIT
        add_node_best_fit(freelist, node);
      case 3: // WORSTFIT
        add_node_worst_fit(freelist, node);
    }
  }
  else {
